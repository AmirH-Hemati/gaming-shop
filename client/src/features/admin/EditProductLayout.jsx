...
function EditProductLayout() {
  ...
  return (
    <div className="flex-1 flex flex-col  w-full overflow-hidden  text-sm md:text-base text-[#192938] font-semibold">
      <div className="w-full font-semibold md:mt-8 grid grid-cols-[1fr_2fr_1fr_1fr] gap-3  bg-[#6366F1] text-white  md:gap-6 rounded-t-md p-4 border-2 border-black/10">
        <p>عکس</p>
        <p>نام محصول</p>
        <p>قیمت</p>
        <p>تنظیمات</p>
      ...
      <ul className="  w-full flex flex-col overflow-y-auto h-full md:h-full    ">
        {products?.data?.map((product) => (
          <li
            key={product?._id}
            className="odd:bg-white even:bg-[#6366F1] even:text-white grid grid-cols-[1fr_2fr_1fr_1fr] gap-3 w-full   px-4 py-6 md:gap-6 border-2 border-black/10"
          >
